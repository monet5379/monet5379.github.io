...
세션에 무엇을 장착했는가(런 빌드) → 이 프레임에 무엇을 시도·적용하는가(액션 척추) → 방·보상을 언제 닫는가(스테이지 루프)를 한 축으로 둡니다.
...
- **Gear**: 획득 즉시 상시 Buff/Passive 또는 Active로 반영합니다. 일부는 Core 레벨 신호만 보내고, Core 내부·피해 식은 복제하지 않습니다. Active·Hyper 실행 슬롯은 액션 척추 Execute 순서에 붙습니다.
...
한 프레임 의도 → 가능 여부 → 타격을 Player·AI가 같은 순서로 타게 했습니다. [드래곤 이즈 데드]({{ "/projects/dragon-is-dead/" | relative_url }})의 Ability·전투 층과 문제의식은 통하고, 손잡이(변신 무기·탄약·차지)가 달라 척추 모양이 다릅니다.
...
스테이지 수명과 방 안 전투 루프·목표·진행 플래그를 나눴습니다. Room은 언제 싸우는가이고, 액션 척추는 한 프레임에 무엇을 시도하는가입니다.
