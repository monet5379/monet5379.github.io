...
role: 세이브 레이아웃
excerpt: "슬롯당 메인·선택적 사이드와 공유 메타로, 세이브 손상·복구·스키마 경계를 타이틀에서 분리한 Unity 케이스 스터디입니다."
mermaid: true
---
...
한 문장으로: 프로필마다 메인을 두고, 필요하면 슬롯당 사이드를 붙이며, 메타로 선택을 기록합니다.
...
- **메인**: 슬롯당 영구 진행 — 쿨다운·pending 저장
- **사이드**: 짧은 진행·스냅샷 — `SaveSide` / `InvalidateSide` (메인과 분리). 사이드만 ≠ 세이브 있음
- **메타**: 선택 슬롯·시퀀스 — 세트와 함께 이동
- **허브**: 메타 + 메인. `Backup/` ≠ 사이드
...
| 축 | 선택 |
|----|------|
| 디스크 | 메타 + Profile0~2 메인 + 슬롯당 선택적 사이드 (Dev는 `_Dev`) |
| 메인 저장 | 쿨다운 + 지연 저장 → 선택 프로필 메인 → 메타 |
| 사이드 저장 | 즉시 `SaveSide` / `InvalidateSide` / `ClearSide` — 메인과 한 호출에 섞지 않음 |
| 로드 | 메타와 프로필 메인을 조립. 사이드는 `TryGetSide` |
| 실패 | 손상·빈 메인 또는 사이드만 백업 후 null, 다른 슬롯 유지 |
...
- **`runs/` 트리**나 사이드를 넘는 런 전용 제품 API가 아닙니다 — 세션·오토세이브는 사이드 레시피 또는 타이틀 필드로 확장합니다.
- 레거시·타 포맷 세이브 **이관**이 아닙니다.
- Excel→Json·SO **고정 정의 Facade**가 아닙니다 (별도 OSS).
- Profile Rot·AES·Steam Cloud merge 전체 패리티가 아닙니다.

증명하려는 것은 동적 진행을 **슬롯 레이아웃**(메인·사이드·메타)으로 어떻게 안전하게 유지·복구·진화시키는가입니다.
