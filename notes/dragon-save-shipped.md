...
| **필수 세이브** | 쿨다운 중에도 반드시 남기는 별도 기록 | 필수 세이브 파일 ×3 |
...
**필수 세이브(×3).** 메인과 **별도 필수 저장 파일**을 같은 방식으로 로테이션합니다. 반드시 남겨야 하는 순간에는 메인과 같은 내용을 필수 경로에 **추가 기록**합니다.
...
저장 요청은 게임 루프에서 **보류분을 처리**합니다. 평상시 일반 저장은 쿨다운 1초와 보류 큐를 타고, 반드시 남겨야 할 순간에는 쿨다운 중이어도 메인과 필수 세이브에 **필수 저장**합니다. 쿨다운이 지나면 루프가 보류분을 기록합니다.
...
- **슬롯 로테이션** — 직전 세대 파일 잔존
- **필수 세이브** — 쿨다운 중에도 중요 순간 저장
- **복구 체인** — 크래시 후 진행 복구 시도
- **이중 마이그레이션** — EA 패치 후 구 세이브
- **밸런스 Json·프로필 분리** — 데이터·진행 경계

**한계:** 영구 진행·필수 세이브·실패 백업·선택 슬롯이 **같은 타이틀 코드**에 있습니다. 디스크를 메인·사이드·메타로만 설명하기 어렵습니다 — [2편]({{ "/notes/save-layout-boundaries/" | relative_url }})이 그 분리 시도입니다.

## 정리

드래곤 이즈 데드 출시 세이브는 **로테이션·필수 세이브·복구 체인·이중 마이그레이션**으로 라이브에서 진행 유실을 막은 기록입니다. 완벽한 레이아웃 분리가 아니라, 소규모 팀이 게임 세이브 한곳에 저장·복구·스키마를 모아 출시까지 버틴 형태입니다.
