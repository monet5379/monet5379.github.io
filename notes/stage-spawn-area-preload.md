...
[드래곤 이즈 데드]({{ "/projects/dragon-is-dead/" | relative_url }}) 스테이지 이동 최적화에서 적용한 내용입니다. **플레이어·QA 증상:** 같은 지역 안에서 게이트로 이동할 때마다 짧은 끊김(히치)이 반복되는 경우.

## 맥락

월드는 **지역** 여러 개로 나뉘고, 각 지역은 **스테이지 프리팹** 여러 개를 가집니다. 플레이어는 **게이트**로 스테이지 사이를 이동하며, 지역이 바뀌는 이동과 지역 안 이동을 구분합니다. 이 노트는 **스테이지 스폰** — 스테이지 인스턴스를 언제 만들고 언제 켜는지 — 정책입니다.

예: 지역 A에서 스테이지 1→2→3을 탐험할 때, 게이트마다 Instantiate·로드가 끼면 히치가 반복됩니다. 비용을 **이동마다**가 아니라 **지역 진입 한 번**으로 옮기는 것이 목표입니다.

| 용어 | 의미 |
|------|------|
| inactive root | 비활성 스테이지를 모아 두는 부모. 계층은 유지하되 업데이트·연출을 끔 |
| Spread | 일괄 preload의 **프레임 분산**. preload 정책 자체를 바꾸지 않음 |
| Resident 메모리 | preload로 상주하는 스테이지 인스턴스 메모리 |
| 저사양 마일스톤 | 콘솔·저사양 빌드 이후 후보(Addressables 스트리밍 등) |

## 문제

스테이지 게이트를 지날 때마다 해당 스테이지를 Instantiate·로드하던 방식은, 지역 안을 탐험하는 동안 로딩과 히치가 반복되었습니다.

| 증상 | 원인 |
|------|------|
| 스테이지 이동마다 로딩·대기 | 이동 시점에 해당 스테이지만 생성 |
| 지역 내 탐험이 끊김 | 히치가 게이트 통과·플레이 중 입력과 겹침 |
...
일괄 preload와 별도로, 지역 진입 한 프레임 히치를 줄이기 위해 간격을 두고 스폰할 수 있습니다. 활성화할 스테이지를 먼저 스폰·활성하고, 나머지를 프레임 간격으로 올립니다. Spread는 preload 정책을 대체하지 않고, 비용을 프레임에 나눌 뿐입니다.

현재는 Standalone·Editor에서 스테이지 1개 / 프레임 1로 분산합니다. Spread가 꺼진 플랫폼은 동기 일괄 스폰이라 진입 비용이 그대로일 수 있습니다.

## 기각·보류

**스테이지마다 on-demand 생성** — 이동마다 로딩이 납니다. 기각했습니다.

**전 월드 일괄 preload** — 메모리·부팅 비용이 큽니다. 지역 단위만 허용합니다.

**동일 지역도 매번 씬 리로드** — 불필요 히치가 납니다. 기각했습니다.

**가드 없이 preload만** — 비활성 사이드이펙트·재활성이 깨집니다. 기각했습니다.

**Addressables 스트리밍·거리 기반 로드** — 콘솔·저사양 마일스톤 이후 후보로 보류합니다.

## 한계

Spread가 꺼진 플랫폼은 동기 일괄 스폰이라 **지역 진입 비용이 한 프레임에** 남을 수 있습니다. preload는 동일 지역 **게이트 히치**를 없앨 뿐, 진입 시간·Resident 메모리 대가는 그대로입니다.

측정·합격은 Win64·Editor proxy 중심으로 두고, Switch 등 **실기기 수치 없이 「최적화 완료」로 닫지 않습니다.** Addressables 스트리밍·거리 기반 로드는 이 정책 밖입니다.
...
- 동일 지역 게이트 이동: 재생성 없이 전환, 긴 로딩·Instantiate 히치 없음
