...
**Exit는 Idle로 돌아오는 동시에 이동 잠금·입력 버퍼가 풀리는 지점입니다.** 상태에 들어갈 때 클립 길이를 기준으로 이동·좌우 반전을 잠그고, 나갈 때 잠금을 풀고 스킬 쪽에 종료를 알리며 대기 중인 입력을 소비합니다. “스킬이 끝났는데 안 움직인다”, “연타가 씹힌다”가 대부분 이 지점입니다. 액션을 이동 그래프에 직접 이어 붙이면 이 해제 시점이 상태마다 흩어집니다.
