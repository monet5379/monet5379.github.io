...
허브 디스크 정본은 **Meta + Profile Main**입니다. Side만으로는 «세이브 있음»이 아니고, 손상·빈 파일은 그 파일만 `Backup/`으로 치운 뒤 null로 둡니다. `Backup/`과 Side의 차이는 [2편 (노트)]({{ "/notes/save-layout-side-lane/" | relative_url }})입니다.
