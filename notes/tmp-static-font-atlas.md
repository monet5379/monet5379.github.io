---
layout: page
title: TMP 정적 아틀라스로 동적 히치 피하기
permalink: /notes/tmp-static-font-atlas/
date: 2026-07-23
excerpt: "TMP 동적 폰트의 런타임 아틀라스 성장·프레임 히치를 피하기 위해, 문자열 JSON에서 언어별 고유 글자를 뽑아 정적 아틀라스에 넣는 파이프라인을 정리합니다."
tags: [폰트]
series: tmp-font
series_title: TMP 폰트
series_order: 1
...
TMP 동적 폰트의 런타임 아틀라스 성장·프레임 히치를 피하기 위해, 문자열 JSON에서 언어별 고유 글자를 뽑아 정적 아틀라스에 넣는 파이프라인을 정리합니다.
...
| 말 | 역할 |
|----|------|
| **동적 아틀라스** | 런타임에 글자를 추가하며 아틀라스가 커짐 (히치·상한 불명) |
| **정적 아틀라스** | 빌드 전에 글자 집합을 확정해 아틀라스에 넣음 |
...
`String*.json`(로컬라이즈 문자열) → 정제 · 언어 버킷별 unique codepoint → 생성 텍스트/테이블 → TMP Font Asset Character Table(정적) → 런타임은 언어에 맞는 Font Asset 선택

## 문제

TextMesh Pro의 동적 Font Asset은 처음 보는 glyph가 요청될 때 런타임 아틀라스를 키웁니다. 다국어·CJK에서는 다음이 반복됩니다.
...
배포 기본은 정적으로 두고, 필요한 글자 집합은 에디터에서 문자열 데이터로부터 결정적으로 뽑는 쪽으로 바꿨습니다.
...
문자열 JSON에서 버킷별 글자를 뽑아 정적 Character Table에 넣는 경로입니다. Warmup sample이나 동적 아틀라스가 glyph SSOT가 아닙니다.
...
UI·시스템 문자열과 대화를 나눈 이유는, 대화 전용 대량 CJK가 UI 폰트 아틀라스를 불필요하게 키우기 때문입니다. 유지보수 단위는 **언어/버킷별 정적 Font Asset**입니다.

**드래곤 이즈 데드 배포 vs 공개 Demo.** 위 European **합집합**은 드래곤 이즈 데드 출시 기준입니다. 오픈소스 [TMP 폰트 파이프라인]({{ "/projects/tmp-font-pipeline/" | relative_url }}) Demo는 EN · FR · DE · IT · ES를 **언어별 버킷·정적 asset**으로 분리합니다. UI·Dialogue 분리 원칙은 같고, 유럽어 쪼개기만 다릅니다.
...
**Warmup sample만으로 glyph 보장** — sample은 일부만 커버합니다. 정적 추출이 SSOT입니다.

**Dialogue를 default 셋에 합치기** — UI atlas가 불필요하게 커집니다. 버킷 분리를 유지합니다.

## 한계

Static은 **추출에 들어간 문자열 JSON의 글자**만 보장합니다. workbook·JSON이 바뀌면 Extract → 적용를 **같이** 돌리지 않으면 **tofu(□, 폰트에 없는 글자)**나 누락 현상이 생깁니다. 여기서 tofu란, 폰트에 해당 글리프가 없어서 네모(□)로 표시되는 것을 말합니다. 런타임에 처음 보는 글자는 Dynamic처럼 자동으로 추가되지 않습니다.

첫 그리기 머티리얼·mesh 스파이크는 이 파이프라인이 없애지 않습니다 — [워밍업]({{ "/notes/tmp-font-warmup/" | relative_url }})이 담당합니다. Warmup sample만으로 glyph 완결을 기대해서는 안 됩니다.

## 문자열 갱신 후

String workbook / JSON이 바뀌면 정적 폰트도 같이 갱신합니다.
...
드래곤 이즈 데드에서 정리한 추출 → 정적 bake → 런타임 font 선택을 게임 없이 복사해 쓸 수 있게 [TMP 폰트 파이프라인]({{ "/projects/tmp-font-pipeline/" | relative_url }}) (`unity-tmp-font`)로 뺐습니다. Install·API·Demo 절차는 [GitHub README](https://github.com/monet5379/unity-tmp-font)가 정본입니다.

Editor Window **Extract**는 `String*.json` → `unique_chars_*.txt`(sanitize), **Apply** + `FontAtlasApplyProfile`은 Generated txt → 정적 SDF(2048 bake)입니다. `FontRoleCatalog`가 `LanguageId` + **Ui** / **Dialogue** → `TMP_FontAsset`을 잇고, `Assets/Demo` SampleScene은 Extract/Apply 놀이터입니다.
...
다국어 TMP에서 동적은 “일단 돌아가게”는 쉽지만, 출시·라이브 기준으로는 히치와 메모리 상한이 문제입니다. 문자열 데이터를 문자셋의 SSOT로 두고 정적으로 고정하면, 폰트·로컬라이즈·워밍업 각각의 책임이 명확해집니다.
